...
| `rtl/top/wrap/ccv_<blk>_w.sv` | hardening wrappers: each block and its channel ends' repeaters, from `params/links.json` ([`physical.md`](physical.md)) |
| `rtl/top/dpi/ccv_<blk>.sv`, `ccv_ext.sv` | DPI shims: the C++ skeleton's blocks, same module names and port lists |
| `test/top/tb_sv_hosted.sv` | the top built from the shims, run to the end of a kernel |
...
the C++ skeleton's block swap.

**The top is instances and nets, nothing else** (`rtl-coding-style.md`,
"The top level"; decided 2026-09-26).
- **No top-level logic of any kind:** no gate, flop, constant or clock gate.
  Each block takes `core_clk` ungated and gates it inside itself, on its own
  quiescence or stall, so what decides to sleep never leaves the block. The
...
**Ports are per slot, the payload typed.** Every channel signal is
`<chan>[_c<NN>][_s<K>]_<sig>`, with the payload as `ccv_<chan>_t`, and wake
is `<chan>[_c<NN>]_wake` per channel instance (`rtl-coding-style.md` has
the rule). Each suffix appears only where its dimension exists. The first
version was one flat vector per signal across all slots and copies. That
was correct but unreadable, and the constraint behind it only rules out an
array of structs at a port: Yosys 0.33 mis-elaborates that silently (CCV-L26).
The wiring check now also refuses the Yosys warnings that give it away.
With stubs, the top proves structure; with the DPI shims it runs the C++
skeleton's blocks, below.
...
- **Wake is not a fabric.** It is `_wake`, a fifth channel signal from sender
  to receiver, one per channel instance. The receiver's clock gate is inside
  the receiver, so nothing about sleep crosses the top except each block's
  `clk_gated`, an observation for the checker bank under `CCV_CHECK`.
- **CRU owns the CSR fabric,** as a star: it drives every other block's
  `csr_req` and gathers their `csr_rsp` and `csr_credit`, and its own CSR
  ports are the host side, at the top boundary. Its vectors have no slot for
  CRU itself, so nothing at the top is tied off.
...
### C++ blocks hosted by the SV top

The swap from C++ to RTL happens one block at a time. Each step puts a
Verilog block beside C++ ones, so the boundary between them has to be the SV
port list. `rtl/top/dpi/` makes that concrete in the other direction: a shim
per block type, with the stub's module name and the same generated port list,
whose body runs the C++ skeleton's block through DPI-C
(`sim/skel/dpi_host.cpp`). Built from shims alone, `ccv_core_top` is the
whole C++ machine stitched by the generated Verilog. Every connection between
two blocks is a net in the top, and the checker bank in the top judges them.

**One call, one block, one cycle.** At each edge of its clock, a shim
samples every channel signal at its ports, including its own registered
outputs, since a sender reads back its valid and a held payload. The host
writes that into the slots' `cur`. It resets the fields this block drives to
what `Slot::begin()` gives them, runs the block's cycle, and hands back what
it drove. The shim registers it. Both ends of a slot sample the same net at
the same edge, so they write the same `cur`, and each writes only its own
half of `nxt`. No block reads `nxt`, so the simulator's order of calling
shims can't matter: `channel.h`'s two-phase argument, now made by the
Verilog. The `Machine` is still built for the `Sender`/`Receiver` objects the
blocks' ports point at, the finish rule's in-flight count and the overflow
total. Its `step()` is never called, because the SV registers are the commit.

**Checked** by `tools/check-sv-hosted.sh` (Verilator only, since Icarus has
no DPI-C):

- **Each S1 kernel:** the KERNEL, UNUSED and XFER lines are identical to the
  C++-hosted run's. The event trace is the same multiset of records, cycle by
...
- **Each of the 15 kernel controls:** the same KERNEL line from both hosts,
  ...
  bank in the top).
- **The partner is a skew control:** `+ccv_shim_delay=<instance>` puts one
  extra register on one shim's outputs. Every port still obeys the protocol
  (the bank reports nothing for DEC), so only the comparison can see it.
  DEC late differs in 223 cycles; one lane of 32 late differs in 173. A skew
  on a block the kernel never uses (SYU in `vadd`) is invisible, correctly,
  so the controls target blocks on the path.
- The shims are `-Wall` clean. They refuse to elaborate without `CCV_TRACE`,
  because C++ blocks carry trace identity on every message.

The KERNEL line and the finish rule come from shared code (`KernelEnd`,
`printKernelReport` in `kernel.cpp`), so the two hosts can't drift apart in
how they report.

**What this doesn't do yet:**

- **Mixed hosting.** A real RTL block beside shims needs no new mechanism on
