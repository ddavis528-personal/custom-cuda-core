...
- **`_wake` is not sampled.** No C++ block sleeps (Q-33), so a shim drives
  wake low and `sleep_ok` low, as a stub does.
- **Shims clock on `clk`,** the gated clock, as real RTL will. `sleep_ok` is
  low, so it runs every cycle. The exact match is also evidence that
  derived-clock always-blocks and the bank on `core_clk` sample the same edge
  without a race under Verilator 5.020.
