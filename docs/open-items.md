...
- *S1*: running `vadd`.
- *review n*: the n-th skeleton review response ([`skeleton.md`](skeleton.md)). Review 4 is the response to this register.
...
| Q-21 One predicate field for guard and destination | S1 | payload (this repo) | **open — a payload fix, not an ISA question.** Schema: `pred_src_and_dst`. The ISA encodes the two separately: Format C/C′ carry the predicate qualifier (guard index + negate) at `[29:27]` and the predicate destination at `[31:30]` (ISA v1.6 §3). Only the uop conflates them, in one `pred_reg`, so `@P0 setp P1, …` can't be carried. It's the same one-field-two-meanings pattern as the branch length folded into `imm`. The fix is local: split `pred_reg` into `pred_guard` with `pred_neg`, and `pred_dst` with `pred_we`. S1 refuses a record whose guard and destination differ, and vadd only uses P0 |
...
| Q-32 RCU op-class boundary cases | review 2 | — | **closed (2026-09-25) — both in RCU, under a rule that needs no special case: the lane executes anything that reads lane data; RCU executes everything that reads no lane data, plus the horizontal ops.** Branch resolution reads predicates and produces control state, touching no GPR in either direction, and the predicate values exist only in RCU. `pmov` has no source and a predicate destination, and a lane can't write the predicate file, so a round trip through the lane would do nothing. The rule keeps `setp`, `add.pp` and `cas` in the lanes, since they read lane data even though they write predicates, which is what `pred_out` and `pred_result` are for. It also moves three built ops: Q-38 |
...
| Q-38 The Q-32 rule moves `movi`, `movi48` and `srd` to RCU | review 4 | you | **awaiting confirmation.** All three read no lane data. `movi` and `movi48` have only an immediate, and `srd` reads a special register (lane identity or launch state, both known to RCU, and neither in the register file). So by the rule RCU writes their result into the register file directly, with no lane round trip. That's the same argument as `pmov`'s, applied to a GPR destination. As built, S1 still sends all three to the lanes, which contradicts the rule, so the stubs list them as the one pending exception and check everything else against it. Confirm, and they move |
