...
//
// A slot is one of `rate` independent credited channels. That rate > 1 means
// independent slots is an ASSUMPTION -- see docs/skeleton.md.
//===----------------------------------------------------------------------===//
...
public:
  ...
  bool cycle(bool launch, const Bits &msg) {
...
public:
  ...
  const Bits &front() const { return buf_.front(); }
